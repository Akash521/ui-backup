...
function view(e) {
  ...
  if(userStatus == "super_administrator" || userStatus == "helpdesk"){
    $(".assignoptions").show();
  ...
  if (true) {
    ...
    if (videojs.getPlayers()["my_video_1"]) {
      ...
      $(".videojs_mp4").append(
        '<video loop id="my_video_1" muted autoplay poster="'+ thumbnail +'" class="video-js vjs-default-skin" controls preload="none" style="width:100%;min-height: 60vh;"\n' +
          "               data-setup='{}'>\n" +
          "            <source src=" +
          event_url +
          "  type='video/mp4'>\n" +
      ...
      $(".videojs_mp4").append(
        '<video loop id="my_video_1" autoplay poster="'+ thumbnail +'" muted class="video-js vjs-default-skin" controls preload="none" style="width:100%;min-height: 60vh;"\n' +
          "               data-setup='{}'>\n" +
          "            <source src=" +
          event_url +
          "  type='video/mp4'>\n" +
  ...
  if (viewflag) {
    ...
    if (
      ...
      $.post("/changealertname", alertnameobj, function (res) {
        ...
        if (res.status == "Successfully changed") {
...
function addMQTTresponse(data) {
  ...
  if (
    ...
    if (document.getElementById("count_span_" + data.priority)) {
      if (
        !$(".count_" + data.priority)
          .parent()
          .hasClass("active")
      ) {
        $("#count_span_" + data.priority)
      ...
      if (
        ...
        $(".count_" + data.priority).append(
          '<span class="label label-danger" style="font-size: 10px;font-weight: 400;position: relative;right: -5px;padding: 1px 5px;" id=' +
            id_count +
            ">1</span>"
        );
        (function blink() {
    ...
    columns: [
      {
        ...
        render: function (data, type, row) {
          ...
          return `<a onclick='view(this)' class='fw-semi-bold alert${row.read_flag}' style='cursor: pointer'>
              ${event_name}</a>`;
        },
      },
      ...
      },
     {
        ...
        className: "hidden-xs",
        render: function (data, type, row) {
          console.log($($("#" + row.alert_id).children()[5]))
