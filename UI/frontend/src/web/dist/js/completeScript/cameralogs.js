...
cameralogs = function () {
    ...
    $('#breadCrumbsLogs').append('<span>Logs &nbsp; <i class="fa fa-angle-right fa-lg" style="position: relative;top: -2px;right: 3px;font-size: 10px;"></i> Cities &nbsp; <i class="fa fa-angle-right fa-lg" style="position: relative;top: -2px;right: 3px;font-size: 10px;"></i> Areas </span> &nbsp;');
    ...
    var settings = {
        ...
        "url": "http://"+base_domainip+"/event-app/get_alerts_by_plant/saurabh/saurabh/"+localStorage.getItem('camDXBLogsID')+"/"+localStorage.getItem('cityAreaName'),
        "method": "GET",
        "headers": {
    ...
    $.ajax(settings).done(function (response) {
        ...
        // }
        $('#tble_cc').append('<table id="datatable-table" class="table table-striped table-hover"> <thead> <tr> <th>Id </th> <th>Alert Name</th> <th class="no-sort hidden-xs">Street </th> <th class="hidden-xs">Time </th> <th class="hidden-xs">Preview</th></tr> </thead> <tbody id="allcamsDXB" ></tbody> </table>');
        if(response.plantwise_alerts.length>0){
            console.log("here");
            var count=0;

            for (i = 0; i < response.plantwise_alerts.length; i++) {


                var event_name ;
                if(response.plantwise_alerts[i].alert_2 =="" ){
                    event_name =response.plantwise_alerts[i].alert_1;
                }else {
                    event_name =response.plantwise_alerts[i].alert_1 + " & " + response.plantwise_alerts[i].alert_1
                }

                var cityName = response.plantwise_alerts[i].city+", "+response.plantwise_alerts[i].state;

                count+=1;

                $("#allcamsDXB").append('<tr><td>'+count+'</td><td><a class="fw-semi-bold "    style="cursor: pointer">'+"#" + response.plantwise_alerts[i].alert_1 + '</a></td> <td class="hidden-xs"> <span class="">'+response.plantwise_alerts[i].workshop+'</span> </td> <td class="hidden-xs"><span >'+response.plantwise_alerts[i].date+'</span></td>  <td> <span flight_id="'+response.plantwise_alerts[i].alert_id+'" airport="'+cityName+'"  terminal="'+response.plantwise_alerts[i].plant+'"  stand_type="'+response.plantwise_alerts[i].workshop+'"  camName="'+response.plantwise_alerts[i].cam_name+'"  camera_id="'+response.plantwise_alerts[i].cam_name+'"  event_name="'+event_name+'"  event_url="'+response.plantwise_alerts[i].video+'" event_time="'+response.plantwise_alerts[i].date+'" status="'+response.plantwise_alerts[i].alert_status+'" class="getCamAlertsDXB" style="text-decoration:underline; cursor: pointer; color:#8ec5fd;"> Preview</span> </td></tr>');
