...
(function (_PruneCluster) {
  ...
  var Cluster = (function (_super) {
    ...
    Cluster.prototype.ApplyCluster = function (newCluster) {
      ...
      this.averagePosition.lat =
        (this.averagePosition.lat * currentTotalWeight +
          newCluster.averagePosition.lat * weight) /
        newWeight;

      this.averagePosition.lng =
        (this.averagePosition.lng * currentTotalWeight +
          newCluster.averagePosition.lng * weight) /
        newWeight;

      this.population += newCluster.population;
      this.totalWeight = newWeight;

      this.bounds.minLat = Math.min(
        this.bounds.minLat,
        newCluster.bounds.minLat
      );
      this.bounds.minLng = Math.min(
        this.bounds.minLng,
        newCluster.bounds.minLng
      );
      this.bounds.maxLat = Math.max(
        this.bounds.maxLat,
        newCluster.bounds.maxLat
      );
      this.bounds.maxLng = Math.max(
        this.bounds.maxLng,
        newCluster.bounds.maxLng
      );
      ...
      if (Cluster.ENABLE_MARKERS_LIST) {
        this._clusterMarkers = this._clusterMarkers.concat(
          newCluster.GetClusterMarkers()
        );
      }
    };
  ...
  var PruneCluster = (function () {
    ...
    PruneCluster.prototype._sortMarkers = function () {
      ...
      if (
        this._nbChanges &&
        (!length || this._nbChanges / length > ratioForNativeSort)
      ) {
    ...
    PruneCluster.prototype.ProcessView = function (bounds) {
      var heightBuffer =
          Math.abs(bounds.maxLat - bounds.minLat) * this.ViewPadding,
        widthBuffer =
          Math.abs(bounds.maxLng - bounds.minLng) * this.ViewPadding;
...
var PruneClusterForLeaflet = (L.Layer ? L.Layer : L.Class).extend({
  initialize: function (size, clusterMargin, spiderfyDistanceMultiplier) {
    var _this = this;
    if (typeof size === "undefined") {
      size = 120;
    }
    if (typeof clusterMargin === "undefined") {
      clusterMargin = 20;
    }
    if (typeof spiderfyDistanceMultiplier === "undefined") {
    ...
    this.spiderfier = new PruneClusterLeafletSpiderfier(this, spiderfierOpts);
    ...
    m.on("click", function () {
      ...
      if (b) {
        ...
        var zoomLevelBefore = _this._map.getZoom(),
          zoomLevelAfter = _this._map.getBoundsZoom(
            bounds,
            false,
            new L.Point(20, 20)
          );
  ...
  PrepareLeafletMarker: function (marker, data, category) {
    if (data.icon) {
      ...
      $(marker).ready(function () {
        $("." + marker.options.icon.options.className).attr(
          "data-toggle",
          "tooltip"
        );
        $("." + marker.options.icon.options.className).attr(
        ...
        // alert(marker.options.icon.options.area);

        $("." + marker.options.icon.options.className).attr(
          "data-original-title",
          `
          <span><b>Location:</b> ${capitalizeFirstLetter(
            marker.options.icon.options.area
          )}</span>`
        );
      ...
      $(marker)
        .unbind("click")
        .click(function (e) {
          // $('.nav-left').css({"transform":"translatex(0)", "z-index":"999", "transition": "all 0.5s ease-in-out"});
          openleft(
            e.target.options.icon.options.className.replaceAll("__", " ")
          );
          $(".ripple").remove();
          $(marker._icon).prepend(`<span class='ripple'></span>`);
    ...
    if (data.popup) {
      var content =
        typeof data.popup === "function"
          ? data.popup(data, category)
          : data.popup;
      if (marker.getPopup()) {
      ...
      //     // alert('marker');
      //     $("#nome").empty();
      //     $("#nome").append(data.title);
      //     // $("#nav").empty(data.title);
      // });
    }
  },
  onAdd: function (map) {
  ...
  ProcessView: function () {
    ...
    for (i = 0, l = clusters.length; i < l; ++i) {
      ...
      var latMargin =
          (icluster.bounds.maxLat - icluster.bounds.minLat) * marginRatio,
        lngMargin =
          (icluster.bounds.maxLng - icluster.bounds.minLng) * marginRatio;
    ...
    clusters.forEach(function (cluster) {
      ...
      var position = new L.LatLng(
        cluster.averagePosition.lat,
        cluster.averagePosition.lng
      );
      ...
      if (oldMarker) {
        if (
          ...
          cluster.hashCode === oldMarker._hashCode
        ) {
          if (
            resetIcons ||
            oldMarker._zoomLevel !== zoom ||
            cluster.lastMarker.data.forceIconRedraw
          ) {
            _this.PrepareLeafletMarker(
              oldMarker,
    ...
    for (i = 0, l = clusterCreationList.length; i < l; ++i) {
      ...
      if (icluster.population === 1) {
        creationMarker = this.BuildLeafletMarker(
          icluster.lastMarker,
          iposition
        );
      } else {
...
var PruneClusterLeafletSpiderfier = (L.Layer ? L.Layer : L.Class).extend({
  ...
  initialize: function (cluster, options) {
    ...
    if (options) {
      this.spiderfyDistanceMultiplier =
        options.spiderfyDistanceMultiplier || this.spiderfyDistanceMultiplier;
    }
  },
  onAdd: function (map) {
    this._map = map;
      ...
      var anim = window.setInterval(function () {
        ...
        for (i = 0, l = points.length; i < l; ++i) {
          ...
          polylines.push([
            center,
            new L.LatLng(
              center.lat + diffLat * stepRatio,
              center.lng + diffLng * stepRatio
            ),
          ]);
        }
