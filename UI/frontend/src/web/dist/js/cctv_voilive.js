...
function newVehicleMQTTAlert(data){
  if(localStorage.getItem("voicamName-DXB") == data.cam_name){
    if(data.auth_vehicle == "Y"){
      $("#vehicleauthounautoheader").css("background-color","green")
    }else{
      $("#vehicleauthounautoheader").css("background-color","red")
  }

  $("#vehicle_number").text(data.voi_details["Vehicle Number"] == "" ? "Unauthorized" : data.voi_details["Vehicle Number"])

//   $("#vehicleauthounautoinfo").empty().append(`${Object.entries(data.voi_details).map(voi=>{
//   if(voi[0]!="Vehicle Number"){
//     return `<span style="text-transform: capitalize;">${voi[0]}: </span> <a>${voi[1]}</a> </br>`
//   }
// }).join("")}`)

      $("#vehicleauthounautoinfo").empty().append(`
      <table id="vehicleinfotable" class="table table-striped table-hover dataTable no-footer" style="width:100%"></table>
      `)

  $("#vehicleinfotable").append(
    `<thead> <tr> <th></th><th></th></tr> </thead><tbody id="vehicleinfobody"></tbody>`
  )

  Object.entries(data.voi_details).forEach(function(data){
    if(data[0] !== "Vehicle Number"){
      $("#vehicleinfobody").append(`<tr style="font-size: 18px;"><td>${data[0]?.replaceAll("_"," ")}</td><td>${data[1]}</td></tr>`)
    }
  })
  ...
  if(window.innerHeight < window.innerWidth){
   ...
   $("#vehicleauthounautoheader").css("transform", "translateY(0)")
   $("#canvas_vid1").css("width","70%").css("height","70%").css("top","30%")
    $("#vehicleauthounautoinfo").css("transform", "translate(0)")
  }else{
    $("#vehicleauthounautoheader").css("transform", "translateY(0)")
   $("#canvas_vid1").css("height","50%").css("top","30%")
    $("#vehicleauthounautoinfo").css("transform", "translate(0)").css("width","100%").css("top","80%")
  }

    setTimeout(()=>{
    $("#vehicleauthounautoheader").css("transform", "translateY(-120%)")
   $("#canvas_vid1").css("width","100%").css("height","100%").css("top","0")
    $("#vehicleauthounautoinfo").css("transform", "translate(110%)")
    }, 12000)
  }
   ...
   var string_text = "";
                  for (var key in data.voi_details) {
                    if (
                      key != "_id" &&
                      key != "voi_id"
                    ) {
  ...
  $(".active-li").removeClass("active-li");
         ...
         function openallalerts(){
          ...
          function loadData(){
            ...
            async: true,
            crossDomain: true,
            url: "/get_cam_alerts_nop?name=" + localStorage.getItem("voicamName-DXB") + "&page="+pageCount,
            // "url": "http://"+localStorage.getItem("DXB_ip")+"/event-app/get_cam_alerts/saurabh/saurabh/cam-test",
            method: "GET",
            headers: {
            ...
            if (responseAlerts.cam_alerts.length > 0) {
              ...
              for (
                ...
                if (
                  ...
                  var string_text = "";
                  for (var key in responseAlerts.cam_alerts[p1].voi_details) {
...
setTimeout(()=>{
  let player;
    ...
    var cameraName = localStorage.getItem("voicamName-DXB")
  ...
  for (i = 0; i < getpoivoicamli.length; i++) {
    for (j = 0; j < getpoivoicamli[i].location.length; j++) {
      for (k = 0; k < getpoivoicamli[i].location[j].camera.length; k++) {
        ...
        if (cameraName == getpoivoicamli[i].location[j].camera[k].cam_name) {
          ...
          $("#canvas_vid1").remove();
          $("#videoDiv1").append(
            '<canvas ondblclick="closenavsidebar()" id="canvas_vid1" style="width:100%;height:100%;position: absolute;left: 0;bottom: 0;transition: all 1s;"></canvas>'
          );
    ...
    $("#videoDiv1").append(
      "<div style='margin-top: 50px;height: 100%;width: 100%;display: flex; justify-content: center;align-items: center;'>No stream found, Please select camera or add VOI service to camera.</div>"
      );
      $(".openallalerts").hide()
      $("#sidebarangle").hide()
    }
...
function closenavsidebar() {
  ...
  if($("#sidebar.sidebar.nav-collapse.collapse.homeside").css("margin-left") == "0px"){
    ...
    $(".widget.user-bdy").css("padding","0px").css("margin-left","0px").css("margin-top","0px").css("margin-right","0px").attr("style",$(".widget.user-bdy").attr("style")+ "margin: 0 !important;")
    ...
    $(".widget.user-bdy").css("padding","15px").css("margin-left","280px").css("margin-top","94px").css("margin-right","15px")
    ...
    $("#nav.side-nav.nav-right.nav_com").attr("style","overflow-y: scroll;overflow-x: hidden; transform: translatex(130%);box-shadow: 2px 2px 20px #000;height: height: calc(100vh - 90px);top:90px;")
    $("#sidebarangle").empty().append('<i class="fa fa-arrows-alt" aria-hidden="true"></i>')
