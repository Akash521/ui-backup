...
var startDate = copyDate(moment(formatDate(localStorage.getItem('playback_start_date'))));
...
return this.each(function () {
  ...
  elem.on('click', function () {
          ...
          for (var i = 0; i < 6; i++) {
              for (var j = 0; j < 7; j++) {
                  ...
                  if (m.month() == selectedMonth.month() && m.weekday() == j) {
                      ...
                      $b.text(day);
                      if ((flagStart && day < dayNow) || ( parseInt(m.format("M")) === parseInt(moment().format("M"))  && day > parseInt(moment().format('D')) )) {
      ...
      function close() {
          ...
          $content.remove();
          $win.remove();
          console.log($('#picker > span:nth-child(1)').text() +" "+$('#picker > span:nth-child(3)').text())
          playbackVideoGet1($('#picker > span:nth-child(1)').text() +" "+$('#picker > span:nth-child(3)').text());

      }

      function nextMonth() {
        
        if (totalMonths(selectDate) < totalMonths(moment())) {
